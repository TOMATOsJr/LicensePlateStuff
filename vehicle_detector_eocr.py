...
class VehicleDetector:
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.info("Loading EasyOCR model for text recognition")
            self.ocr = easyocr.Reader(['en'], gpu=True if self.device == "cuda" else False)
            ...
            if self.device == "cuda":
                ...
                try:
                    ...
                    # Initialize EasyOCR for license plate text recognition on CPU
            ...
            while True:
                ...
                if detected_texts:
                    for i, text in enumerate(detected_texts[:3]):  # Show max 3 texts to avoid clutter
                        info_y += 30
                        # Clean up text for display (EasyOCR may return text with spaces)
                        display_text = text.strip()
                        cv2.putText(annotated_frame, f"Text {i+1}: {display_text}",
    ...
    def _perform_ocr(self, plate_image: np.ndarray) -> dict:
        """
        Perform OCR on a license plate image using EasyOCR.
        ...
        try:
            ...
            # EasyOCR works with RGB images, but OpenCV uses BGR
            if len(plate_image.shape) == 3 and plate_image.shape[2] == 3:
                # Convert BGR to RGB for EasyOCR
                ocr_input = cv2.cvtColor(plate_image, cv2.COLOR_BGR2RGB)
            else:
                # If grayscale, just use as is
                ocr_input = plate_image

            # Perform OCR using EasyOCR
            # EasyOCR returns a list of [bbox, text, confidence] items
            results = self.ocr.readtext(ocr_input)
            ...
            if 'license_plates' in detection and detection['license_plates']:
                ...
                for i, plate in enumerate(detection['license_plates']):
                    ...
                    if 'ocr_text' in plate and plate['ocr_text']:
                        # Format OCR text for display - remove extra spaces from EasyOCR
                        plate_text = plate['ocr_text'].strip()
                        plate_label = f"LP: {plate_text} ({plate['confidence']:.2f})"
                    else:
